...
var PreviewMainMVC = {
    URLs: {
        loadPrams: {
            url: PreviewCommon.baseUrl + "getReportheaderByTypes"
        },
        Serachreport: {
            url: PreviewCommon.baseUrl + "Serachreport"
        },
        LoadReport: {
            url: PreviewCommon.baseUrl + "getReportByTypes"
        },
        loadexcel: {
            url: PreviewCommon.baseUrl + "getReportToExcelByTypes"
        }
    },
    Model: {},
    View: {
        initControl: function () {
            ...
            $("#load_table").click(function () {
                 window.location.href = encodeURI(PreviewMainMVC.URLs.loadexcel.url + "?json=" + JSON.stringify(jsondata) + "&uid=" + uids + "&username=" + usershow("account")+"&type="+PreviewMainMVC.Controller.GetQueryString("type")+"&legion="+usershow("legion")+"&types="+sqlid);
            ...
            $("#befor1").click(function(){
                $(".data").children("button").css("background","#F5F6FA");
                $(".data").children("button").css("color","#0C2D5C");
                $(this).css("background","#0C2D5C");
                $(this).css("color","#ffffff");

                jsondata["startTime"] = getNowBefore(1);
                jsondata["endTime"] = getNowBefore(0);
                $("#startTime").val(getNowBefore(1));
                $("#endTime").val(getNowBefore(0));
                console.log(getNowBefore(1));
            });
            $("#befor7").click(function(){
                $(".data").children("button").css("background","#F5F6FA");
                $(".data").children("button").css("color","#0C2D5C");
                $(this).css("background","#0C2D5C");
                $(this).css("color","#ffffff");
                var date = getNowFormatDate();
                jsondata["startTime"] = getNowBefore(7);
                jsondata["endTime"] = getNowBefore(0);
                $("#startTime").val(getNowBefore(7));
                $("#endTime").val(getNowBefore(0));
            });
            $("#befor14").click(function(){
                $(".data").children("button").css("background","#F5F6FA");
                $(".data").children("button").css("color","#0C2D5C");
                $(this).css("background","#0C2D5C");
                $(this).css("color","#ffffff");
                var date = getNowFormatDate();
                jsondata["startTime"] = getNowBefore(14);
                jsondata["endTime"] = getNowBefore(0);
                $("#startTime").val(getNowBefore(14));
                $("#endTime").val(getNowBefore(0));
            });
            $("#beforele").click(function(){
                $(".data").children("button").css("background","#F5F6FA");
                $(".data").children("button").css("color","#0C2D5C");
                $(this).css("background","#0C2D5C");
                $(this).css("color","#ffffff");
            ...
            $(".input-group-addon").click(function(){
                $(".data").children("button").css("background","#F5F6FA");
                $(".data").children("button").css("color","#0C2D5C");
                $("#beforele").css("background","#0C2D5C");
                $("#beforele").css("color","#ffffff");
            });
        ...
        initData: function () {
           ...
           /* $('#down_' + type).children("a").text(decodeURI(name));*/
            if(uids=="4e6bd2e3-271d-40c0-a08c-c264c14c8409"){
                sqlid=5;
            }else {
                sqlid=7;
            }
                    ...
                    if (data.code == 1) {
                        ...
                        for (var i = 0; i < titleList.length; i++) {
                            ...
                            if (formElement == "text") {
                                ...
                                sql = sql + '<button class="select_button" id="' + defaultValue + '" style="float:left;display: inline-block;color:#0C2D5C;background: #F5F6FA;padding:3px 20px;border-radius: 5px;font-size: 15px;border: 1px solid #666666;margin-left: 15px">' + text + '</button>';
                                $("#button").append(sql);
                                $('#' + uid).css("background", "#0C2D5C");
                                $('#' + uid).css("color", "#ffffff");
                                $("#" + defaultValue).on("click", function () {
                                    $('.select_button').css("background", "#F5F6FA");
                                    $('.select_button').css("color", "#0C2D5C");
                                    $(this).css("background", "#0C2D5C");
                                    $(this).css("color", "#ffffff");
                                    uids = this.id;
                                    $(".delect").remove();
                                    /*  $('#tb_user').bootstrapTable('destroy');*/
                                    PreviewMainMVC.Controller.loadParmsT(PreviewMainMVC.URLs.loadPrams.url, uids);
                                });
                            } else if (formElement == "buttonselect"){
                                sql = sql + '<button class="select_button" id="' + defaultValue + '" style="float:left;display: inline-block;color:#0C2D5C;background: #F5F6FA;padding:3px 20px;border-radius: 5px;font-size: 15px;border: 1px solid #666666;margin-left: 15px">' + text + '</button>';
                                $("#button_select").append(sql);
                                $("#"+defaultText).css("background", "#0C2D5C");
                                $("#"+defaultText).css("color", "#ffffff");
                                $("#" + defaultValue).on("click", function () {
                                    $('.select_button').css("background", "#F5F6FA");
                                    $('.select_button').css("color", "#0C2D5C");
                                    $(this).css("background", "#0C2D5C");
                                ...
                                sql+=' <div style="margin-top: 10px;float: left"> ' +
                                    ...
                                    ' <div class="time-content"> ' +
                                        '<div class="form-group"> ' +
                                            '<div class="input-group date"> <p id="' + names + '" style="display: none">' + defaultValue + '</p>' +
                                    '<input id="' + names + '_input" type="text" class="form-control inputde" value="' + defaultText + '"/>' +
                                    '  <span class="input-group-addon down-ul" style=""> <div class="icon-triangle"></div> </span> </div>' +
                                    ' <ul class="select_ul" id="' + names + '_ul"style="z-index: 10000;position: relative">' ;
                                jsondata[names] = defaultValue;
                                for (var j = 0; j < arraytitle.length; j++) {
                                    if (defaultText == arraytitle[j].name) {
